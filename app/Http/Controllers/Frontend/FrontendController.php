...
$hitdealsbaner = Banner::where(['status' => 1, 'category_id' => 9])
    ...
    public function details($slug)
    {
        $details = Product::where(['slug' => $slug, 'status' => 1])
            ...
            ->firstOrFail();
              ...
              // Related products (same category, exclude current, random order, limit 8)
    $relatedProducts = Product::where('category_id', $details->category_id)
        ->where('status', 1)
        ->where('id', '!=', $details->id) // Exclude current product
        ->with('image')
        ->select('id', 'name', 'slug', 'new_price', 'old_price', 'stock')
        ...
        return view('frontEnd.layouts.pages.details', compact('details', 'products', 'shippingcharge', 'productcolors', 'productsizes', 'reviews','relatedProducts'));
    }
    ...
    {
        $blogs = Blog::where('status',1)->paginate(16);
         $blog_categorys = BlogCategory::where('status',1)->get();
        return view('frontEnd.layouts.pages.blogs', compact('blogs','blog_categorys'));
    }
       public function category_blog(Request $request, $slug)
       {
          $blog_category=BlogCategory::where('slug',$slug)->where('status',1)->first(); 
          $blog_main=Blog::where('blog_category_id',$blog_category->id)->where('status',1)->latest()->paginate(16);
          $blog_categorys = BlogCategory::where('status',1)->get();
             return view('frontEnd.layouts.pages.category_blogs', compact('blog_category','blog_main','blog_categorys'));
       ...
       {
          $blog_categorys = BlogCategory::where('status',1)->get();
          $blog_single=Blog::where('slug',$slug)->where('status',1)->first(); 
          
        //   return response()->json($blog_single);
             return view('frontEnd.layouts.pages.single_blogs', compact('blog_categorys','blog_single'));
    ...
    {
        $blog = CreatePage::where('slug', $slug)->firstOrFail();
