...
class ShoppingController extends Controller
{
    ...
    {
          // Only validate color if product has colors and product has sizes
        $rules = [];
        if ($request->has('has_color') && $request->input('has_color') == 1) {
            $rules['product_color'] = 'required';
        }
        if ($request->has('has_size') && $request->input('has_size') == 1) {
